 ...
 */
export class GitHubReviewPublisher implements IGitHubReviewPublisher {
   ...
   * 发布审查到 GitHub
   */
  async publishReview(reviewOutput: GitHubReviewOutput, prInfo: PullRequestInfo): Promise<void> {
   ...
   */
  private async validateLineComments(
    comments: Array<{ path: string; line: number; body: string; side?: "LEFT" | "RIGHT" }>,
    prInfo: PullRequestInfo
  ): Promise<Array<{ path: string; line: number; body: string; side?: "LEFT" | "RIGHT" }>> {
    const validComments: Array<{ path: string; line: number; body: string; side?: "LEFT" | "RIGHT" }> = [];
   ...
   * 检查是否有权限发布审查
   */
  async checkPermissions(prInfo: PullRequestInfo): Promise<boolean> {
    try {
      // 检查仓库权限
   ...
   * 获取 PR 的当前状态
   */
  async getPRStatus(prInfo: PullRequestInfo): Promise<{
    state: string;
