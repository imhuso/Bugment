...
class BugmentAction {
    ...
    parseDiffContent(diffContent) {
        ...
        core.info(`📊 Diff parsing complete: ${fileCount} files, ${totalHunks} hunks (after applying ignore filters)`);
        ...
        for (const [filePath, hunks] of files.entries()) {
            ...
            hunks.forEach((hunk, index) => {
                core.info(`  📊 Hunk ${index + 1}: lines ${hunk.newStart}-${hunk.newStart + hunk.newLines - 1} (${hunk.lines.length} diff lines)`);
            });
        }
        return { files };
    }
    ...
    async hidePreviousBugmentComments() {
        try {
            ...
            if (reviewsToHide.length > 0) {
                core.info(`📝 Found ${reviewsToHide.length} previous Bugment reviews to hide`);
                let hiddenCount = 0;
                for (const review of reviewsToHide) {
                    try {
                        await this.minimizeComment(review.nodeId);
                        hiddenCount++;
                        core.info(`✅ Hidden review (${review.state}) from ${review.createdAt}: ${review.url}`);
                    }
                    catch (error) {
                        core.warning(`⚠️ Failed to hide review ${review.id}: ${error}`);
                    }
                }
                core.info(`🎯 Successfully hidden ${hiddenCount}/${reviewsToHide.length} previous Bugment reviews`);
            }
            else {
    ...
    isBugmentReview(body) {
        ...
        const bugmentReviewSignature = "🤖 Powered by [Bugment AI Code Review](https://github.com/J3n5en/Bugment)";
    ...
    async markResolvedLineComments(previousReviews) {
        try {
            ...
            for (const thread of reviewThreads) {
                ...
                const hasAIComment = thread.comments.some((comment) => comment.body?.includes("**🐛") ||
